...
class generate_addertree():
  ...
  def printit(self):
    ...
    for num_adder in range(num_adders_in_stage0):
      print("wire   [`DATAWIDTH-1 : 0] add%d_out_stage0;" % output_num)
      print("reg    [`DATAWIDTH-1 : 0] add%d_out_stage0_reg;" % (output_num))
      print("always @(posedge clk) begin")
      print("  if (reset) begin")
      print("    add%d_out_stage0_reg <= 0;" % (output_num))
      print("  end else begin")
      print("    add%d_out_stage0_reg <= add%d_out_stage0;" % (output_num, output_num))
      print("  end")
      print("end")
      print("DW_fp_add #(`MANTISSA, `EXPONENT, `IEEE_COMPLIANCE) add%d_stage0(.a(inp%d),       .b(inp%d),      .z(add%d_out_stage0), .rnd(3'b000),    .status());" % (output_num, input_num, input_num+1, output_num))
      print("")
    ...
    for stage in range(self.num_stages-1):
      ...
      for num_adder in range(num_adders_in_current_stage):
          ...
          print("  end")
          print("end")
          print("DW_fp_add #(`MANTISSA, `EXPONENT, `IEEE_COMPLIANCE) add%d_stage%d(.a(add%d_out_stage%d_reg),       .b(add%d_out_stage%d_reg),      .z(add%d_out_stage%d), .rnd(3'b000),    .status());" % (num_adder_next_stage, stage+1, num_adder_cur_stage, stage, num_adder_cur_stage+1, stage, num_adder_next_stage, stage+1))
          print("")
