...
class generate_max_tree():
  ...
  def printit(self):
    ...
    for num_comparator in range(num_comparators_in_stage0):
      print("wire   [`DATAWIDTH-1 : 0] cmp%d_out_stage0;" % input_num)
      print("reg    [`DATAWIDTH-1 : 0] cmp%d_out_stage0_reg;" % (input_num))
      print("always @(posedge clk) begin")
      print("  if (reset) begin")
      print("    cmp%d_out_stage0_reg <= 0;" % (input_num))
      print("  end else begin")
      print("    cmp%d_out_stage0_reg <= cmp%d_out_stage0;" % (input_num, input_num))
      print("  end")
      print("end")
      print("DW_fp_cmp #(`MANTISSA, `EXPONENT, `IEEE_COMPLIANCE) cmp0(.a(inp%d),       .b(inp%d),      .z1(cmp%d_out_stage0), .zctr(1'b0), .aeqb(), .altb(), .agtb(), .unordered, .z0(), .status0(), .status1());" % (input_num, input_num+1, output_num))
      print("")
    ...
    for stage in range(self.num_stages-1):
      ...
      for num_comparator in range(num_comparators_in_current_stage):
          ...
          print("  end")
          print("end")
          print("DW_fp_cmp #(`MANTISSA, `EXPONENT, `IEEE_COMPLIANCE) cmp0(.a(cmp%d_out_stage%d_reg),       .b(cmp%d_out_stage%d_reg),      .z1(cmp%d_out_stage%d), .zctr(1'b0), .aeqb(), .altb(), .agtb(), .unordered, .z0(), .status0(), .status1());" % (num_comparator_cur_stage, stage, num_comparator_cur_stage+1, stage, num_comparator_next_stage, stage+1))
          print("")
          num_comparator_next_stage = num_comparator_next_stage + 1
          num_comparator_cur_stage = num_comparator_cur_stage + 2
      
    print("DW_fp_cmp #(`MANTISSA, `EXPONENT, `IEEE_COMPLIANCE) cmp0(.a(cmp0_out_stage%d),       .b(ex_inp),      .z1(outp), .zctr(1'b0), .aeqb(), .altb(), .agtb(), .unordered, .z0(), .status0(), .status1());" % (self.num_stages-1))
    print("")
    print("endmodule")
